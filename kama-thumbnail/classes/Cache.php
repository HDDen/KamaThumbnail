...
class Cache {
	...
	public function smart_clear( string $type = '' ): void {
		...
		$_stub = ( $type === 'stub' );
		$cache_dir = str_replace(['$_SERVER["DOCUMENT_ROOT"]', '$_SERVER[\'DOCUMENT_ROOT\']'], $_SERVER["DOCUMENT_ROOT"], kthumb_opt()->cache_dir); // kthumb_opt()->cache_dir
	...
	private function _glob_pattern_by_thumb_url( string $thumb_url ): string {
		...
		return str_replace(['$_SERVER["DOCUMENT_ROOT"]', '$_SERVER[\'DOCUMENT_ROOT\']'], $_SERVER["DOCUMENT_ROOT"], kthumb_opt()->cache_dir) . $mm[0] . '*'; // kthumb_opt()->cache_dir
	}
