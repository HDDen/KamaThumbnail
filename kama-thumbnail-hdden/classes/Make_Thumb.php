...
class Make_Thumb {
	...
	protected function set_thumb_url_and_path(): void {
		...
		$this->thumb_path = str_replace(['$_SERVER["DOCUMENT_ROOT"]', '$_SERVER[\'DOCUMENT_ROOT\']'], $_SERVER["DOCUMENT_ROOT"], kthumb_opt()->cache_dir)     ."/$name_data->sub_dir/$name_data->file_name"; // kthumb_opt()->cache_dir
	...
	protected function create_thumb(): void {
		...
		if( ! $this->check_create_folder() ){
			...
			$msg = sprintf(
				...
				str_replace(['$_SERVER["DOCUMENT_ROOT"]', '$_SERVER[\'DOCUMENT_ROOT\']'], $_SERVER["DOCUMENT_ROOT"], kthumb_opt()->cache_dir)
	...
	protected function change_to_stub( string $path_url, string $type ): string {
		...
		$base = ( 'url' === $type )
			...
			: str_replace(['$_SERVER["DOCUMENT_ROOT"]', '$_SERVER[\'DOCUMENT_ROOT\']'], $_SERVER["DOCUMENT_ROOT"], kthumb_opt()->cache_dir); // kthumb_opt()->cache_dir
